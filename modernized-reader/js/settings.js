...
class SettingsManager {
  ...
  initEventListeners() {
    ...
    if (linesSlider) {
      linesSlider.addEventListener('input', (e) => {
        ...
        if (linesDisplay) {
          ...
          chaptersManager.reloadChapterContent();
        }
      });
    }
    ...
    if (increaseLineHeightBtn) {
      Utils.addClickEvent(increaseLineHeightBtn, () => {
        if (!lineHeightSlider) return;

        const newLineHeight = parseFloat(lineHeightSlider.value) + 0.1;
        if (newLineHeight <= parseFloat(lineHeightSlider.max)) {
          lineHeightSlider.value = newLineHeight.toFixed(1);
          this.updateLineHeight(newLineHeight);
          if (lineHeightDisplay) {
            lineHeightDisplay.textContent = newLineHeight.toFixed(1);
          }
        }
      });
    }
    ...
    if (decreaseLineHeightBtn) {
      Utils.addClickEvent(decreaseLineHeightBtn, () => {
        if (!lineHeightSlider) return;

        const newLineHeight = parseFloat(lineHeightSlider.value) - 0.1;
        if (newLineHeight >= parseFloat(lineHeightSlider.min)) {
          lineHeightSlider.value = newLineHeight.toFixed(1);
          this.updateLineHeight(newLineHeight);
          if (lineHeightDisplay) {
            lineHeightDisplay.textContent = newLineHeight.toFixed(1);
    ...
    const increaseWordSpacingBtn = document.getElementById('increase-word-spacing');
    const decreaseWordSpacingBtn = document.getElementById('decrease-word-spacing');
    const wordSpacingDisplay = document.getElementById('word-spacing-display');
    ...
    if (wordSpacingSlider) {
      wordSpacingSlider.addEventListener('input', (e) => {
        ...
        if (wordSpacingDisplay) {
          wordSpacingDisplay.textContent = wordSpacing;
        }
      });
    }

    if (increaseWordSpacingBtn) {
      Utils.addClickEvent(increaseWordSpacingBtn, () => {
        if (!wordSpacingSlider) return;

        const newWordSpacing = parseInt(wordSpacingSlider.value) + 1;
        if (newWordSpacing <= parseInt(wordSpacingSlider.max)) {
          wordSpacingSlider.value = newWordSpacing;
          this.updateWordSpacing(newWordSpacing);
          if (wordSpacingDisplay) {
            wordSpacingDisplay.textContent = newWordSpacing;
          }
        }
      });
    }

    if (decreaseWordSpacingBtn) {
      Utils.addClickEvent(decreaseWordSpacingBtn, () => {
        if (!wordSpacingSlider) return;

        const newWordSpacing = parseInt(wordSpacingSlider.value) - 1;
        if (newWordSpacing >= parseInt(wordSpacingSlider.min)) {
          wordSpacingSlider.value = newWordSpacing;
          this.updateWordSpacing(newWordSpacing);
          if (wordSpacingDisplay) {
            wordSpacingDisplay.textContent = newWordSpacing;
    ...
    if (fontFamilySelector) {
      const fontOptions = fontFamilySelector.querySelectorAll('.font-option');
      fontOptions.forEach(option => {
        option.addEventListener('click', (e) => {
          ...
          // Get font family from data attribute
  ...
  addRegexReplacementToUI(replacement) {
    ...
    deleteButton.addEventListener('click', () => {
      ...
      this.regexReplacements = this.regexReplacements.filter(
        item => item.match !== replacement.match || item.replace !== replacement.replace
      );
   ...
   * @param {number} lineHeight - New line height
   */
  updateLineHeight(lineHeight) {
    ...
    const chapterContent = document.getElementById('chapter-content');
    if (chapterContent) {
      chapterContent.style.lineHeight = lineHeight;
    }
  }

  /**
   * Update the word spacing
   * @param {number} wordSpacing - New word spacing
   */
  updateWordSpacing(wordSpacing) {
    this.settings.wordSpacing = wordSpacing;

    const chapterContent = document.getElementById('chapter-content');
    if (chapterContent) {
      chapterContent.style.wordSpacing = `${wordSpacing}px`;
    }
  }
  ...
  collectCurrentSettings() {
    ...
    const lineHeightDisplay = document.getElementById('line-height-display');
    if (lineHeightDisplay) {
      this.settings.lineHeight = parseFloat(lineHeightDisplay.textContent);
    }

    // Word spacing
    const wordSpacingDisplay = document.getElementById('word-spacing-display');
    if (wordSpacingDisplay) {
      this.settings.wordSpacing = parseInt(wordSpacingDisplay.textContent);
    }
  }
  ...
  updateSettingsUI() {
    ...
    if (lineHeightSlider) {
      lineHeightSlider.value = this.settings.lineHeight.toFixed(1);
    }

    if (lineHeightDisplay) {
      lineHeightDisplay.textContent = this.settings.lineHeight.toFixed(1);
    }
    ...
    if (wordSpacingDisplay) {
      wordSpacingDisplay.textContent = this.settings.wordSpacing;
    }
  }
  ...
  applySettings() {
    ...
    // Apply word spacing
    this.updateWordSpacing(this.settings.wordSpacing);
